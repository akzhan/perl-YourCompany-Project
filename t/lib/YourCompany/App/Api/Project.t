...
describe "YourCompany::App::Controller::Project" => sub {
    ...
    describe list => sub {
        it "should render empty list" => sub {
            $t->get_ok('/api/projects', $ACCEPT_HEADERS)
                ->status_is(HTTP_OK)
                ->json_is({
                    status  => HTTP_OK,
                    success => true,
    ...
    describe single => sub {
        it "should render not_found on wrong id" => sub {
            $t->get_ok("/api/projects/$WRONG_ID" => $ACCEPT_HEADERS)
    ...
    context "project lifecycle" => sub {
        ...
        describe post => sub {
            it "should create project with unique title" => sub {
                $t->post_ok("/api/projects" => $ACCEPT_HEADERS => json => {
                    title => $new_title,
        ...
        describe "delete existing project" => sub {
            it "should delete ok" => sub {
                $t->delete_ok("/api/projects/$new_id")
                    ->status_is(HTTP_OK)
                    ->json_is({
                        status  => HTTP_OK,
                        success => true,
